...
def plot_client_computations_vs_round_number(
    ...
    global_rounds = sys_metrics[NUM_ROUND_KEY].max()
    ...
    for c in clients:
        ...
        client_computations = [0 for _ in range(global_rounds)]
    ...
    for c_comp_vals in comp_matrix:
        global_rounds = len(c_comp_vals)
        agg_c_comp_vals = []
        for i in range(global_rounds // aggregate_window):
...
def plot_loss_vs_round_number(stat_metrics, train=True, plot_stds=True, weighted=False,
                              figsize=(10, 8), title_fontsize=16, **kwargs):
    """Plots the clients' average test accuracy vs. the round number.
    ...
    plt.title('Accuracy vs Round Number (%s)' % title_weighted, fontsize=title_fontsize)
    if weighted:
        accuracies = stat_metrics.groupby(NUM_ROUND_KEY).apply(_weighted_mean, LOSS_KEY, NUM_SAMPLES_KEY)
        accuracies = accuracies.reset_index(name=LOSS_KEY)
        
        stds = stat_metrics.groupby(NUM_ROUND_KEY).apply(_weighted_std, LOSS_KEY, NUM_SAMPLES_KEY)
    ...
    if plot_stds:
        plt.errorbar(accuracies[NUM_ROUND_KEY], accuracies[LOSS_KEY], stds[LOSS_KEY])
    else:
        plt.plot(accuracies[NUM_ROUND_KEY], accuracies[LOSS_KEY])
...
if __name__ == '__main__':
    ...
    stat_metrics = load_data(stat_file)
    plot_loss_vs_round_number(stat_metrics, weighted=True)
